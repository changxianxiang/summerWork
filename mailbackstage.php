...
//}
$nowtime=time();
$verify = stripslashes(trim($_POST['verify']));// 取回标识码
$sql = "select id,token_exptime,token,status from mailReg where token='$verify'";
$mailresult= mysqli_query($link,$sql);
//var_dump($verify);
//var_dump($mailresult);
$arrs=mysqli_fetch_row($mailresult);
//var_dump($arrs[0]);
if($arrs[1]>0) {
    if ($nowtime > $arrs[1]) { //24hour
...
//    }else{
//        //mysqli_query("update t_user set status=1 where id=".$arrs[1]);
...
if($arrs[3]==1){
    ...
    mysqli_query($link,"update mailreg set status ='1'  where token='$verify'");
}
